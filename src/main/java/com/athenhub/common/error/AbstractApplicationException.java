 ...
 *
 * <p>도메인별 예외 클래스는 이 클래스를 상속받아 사용하며,
 * {@link ErrorCode}를 통해 에러 코드와 HTTP 상태를 포함한다.
 * 또한 메시지 포맷에 필요한 인자(messageArgs)를 함께 전달하여
 * 메시지 파일(MessageSource)에서 최종 사용자 메시지를 생성할 수 있도록 한다.</p>
 *
 * <p>핸들러({@code @RestControllerAdvice})에서는
 * {@link #getCode()} 및 {@link #getErrorArgs()}를 이용해
 * 메시지를 조회하고 응답 본문을 구성한다.</p>
 *
 * <pre>{@code
...
public abstract class AbstractApplicationException extends RuntimeException {
     ...
     * <p>
     * 예: messages.properties 내 문자열
     * {@code "USER_NOT_FOUND=사용자 {0}을 찾을 수 없습니다."}
     * 와 함께 사용됨.
     * </p>
     */
     ...
     * @param errorArgs 메시지 포맷 인자
     */
    public AbstractApplicationException(ErrorCode errorCode, String message, Throwable cause, Object... errorArgs) {
        super(message, cause);
        this.errorCode = errorCode;
        this.errorArgs = errorArgs;
    }
    ...
    /**
     * 에러 코드를 문자열로 반환한다.
     * <p>예: USER_NOT_FOUND, INVALID_PASSWORD 등</p>
     *
     * @return 에러 식별자 문자열
     */
