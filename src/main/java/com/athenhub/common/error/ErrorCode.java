 ...
 *
 * <p>각 도메인은 이 인터페이스를 구현한 열거형(enum)을 통해
 * 에러 식별자(code)와 HTTP 상태(status)를 명시적으로 선언한다.
 * 예외 발생 시 {@link AbstractApplicationException}에서 이 정보를 활용해
 * 응답 상태 코드와 메시지 변환을 일관성 있게 처리한다.</p>
 *
 * <p>일반적인 사용 예:
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>핸들러에서는 {@code getStatus()} 값으로 HTTP 응답 상태를 결정하고,
 * {@code getCode()} 값으로 메시지 리졸버가 사용할 메시지 키를 제공한다.</p>
 *
 * @author 김지원
 * @since 0.2.0
 */
