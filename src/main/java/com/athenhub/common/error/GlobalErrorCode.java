 ...
 *
 * <p>도메인에 종속되지 않는 전역 오류(요청 형식 오류, 인증 실패, 권한 부족 등)에 대해
 * 일관된 HTTP 상태와 에러 식별자를 제공한다. 각 항목은
 * {@link ErrorCode} 인터페이스를 구현하며 전역 예외 처리기에서
 * 응답 상태코드와 메시지 변환에 활용된다.</p>
 *
 * <p>일반적인 사용 예:</p>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>각 에러 코드는 다음 구조를 가진다:</p>
 * <ul>
 *     <li>{@code status}: HTTP 상태 코드</li>
 *     <li>{@code code}: 전역 에러 식별자 문자열</li>
 * </ul>
 *
 * @author 김지원
 * @since 0.2.0
...
public enum GlobalErrorCode implements ErrorCode {
    ...
    /**
     * 잘못된 요청 형식 또는 유효하지 않은 입력으로 인해 요청을 처리할 수 없는 경우.
     * 예: JSON 파싱 오류, 필수 파라미터 누락
     */
    BAD_REQUEST(HttpStatus.BAD_REQUEST, "BAD_REQUEST"),

    /**
     * 입력 값 검증(Validation) 실패.
     * 예: @Valid, @NotNull, @Email 등 제약조건 위반
     */
