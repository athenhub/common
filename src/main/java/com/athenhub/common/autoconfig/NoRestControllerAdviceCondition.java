 ...
 *
 * <p>이 조건은 스프링 컨텍스트 내에
 * {@link RestControllerAdvice} 애노테이션이 붙은 빈이 존재하지 않을 때만 true를 반환한다.</p>
 *
 * <p>즉, 사용자가 직접 전역 예외 처리기(ControllerAdvice)를 구현했을 경우
 * 라이브러리가 제공하는 기본 MvcExceptionHandler 자동 구성(AutoConfiguration)은 비활성화된다.</p>
 *
...
public class NoRestControllerAdviceCondition implements Condition {
     ...
     * @return true — RestControllerAdvice 빈이 하나도 없는 경우
     * false — 하나라도 존재하면 AutoConfiguration 비활성화
     */
    @Override
